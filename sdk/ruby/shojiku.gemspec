...
Gem::Specification.new do |spec|
  ...
  # The second glob is what makes a PLATFORM gem possible at all: the release
  # process drops the prebuilt engine into lib/shojiku/native/ before `gem
  # build`, and a files list of `*.rb` alone would silently ship a platform
  # gem with no engine in it — installable, loadable, and unable to render.
  # In a source checkout the directory does not exist and the glob adds
  # nothing.
  spec.files = Dir["lib/**/*.rb", "lib/shojiku/native/*", "README.md"]
  spec.require_paths = ["lib"]

  # Set ONLY by the release assembly (scripts/release/assemble.sh), which
  # builds this same gemspec once per platform with the matching engine in
  # lib/shojiku/native/. Unset — every checkout, every gate — this is the
  # plain "ruby" platform gem.
  spec.platform = ENV["SHOJIKU_GEM_PLATFORM"] if ENV["SHOJIKU_GEM_PLATFORM"]
