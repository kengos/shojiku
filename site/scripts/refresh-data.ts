...
// halves of the site's generated inputs. CANONICAL WASM = the x86_64 CI
// build (the `wasm-pkg` artifact): the .wasm binary is NOT byte-reproducible
// across host architectures (an arm64 Mac's build differs), so on a non-x86
// host refresh engine/wasm/pkg from the artifact
// (`gh run download <run> -n wasm-pkg -R kengos/shojiku`) before running
// this — a locally built pkg on such a host makes the check cry stale.
// The rest of the generated inputs — site/.data/wasm (copied from the
