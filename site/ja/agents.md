...
テンプレート作成・レンダリングデバッグ・Thinreports移行の3つのスキルがリポジトリに同梱されています。

```bash
npx skills add kengos/shojiku
```

`shojiku-template-author` が本体です。要件からテンプレートを書き、validateし、プレビューの画像を実際に見て、診断がきれいになるまで反復する — その全手順と、リファレンスに書ききれない罠の一覧を持っています。
