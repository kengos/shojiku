---
title: "AIエージェント（MCPサーバー）の設定方法と使い方"
description: "テンプレートはYAMLとJSON、エラーは安定した診断コード、レイアウトはツリーで読み返せます。MCPサーバを登録して、エージェントに書かせて確かめさせるまで。"
---

# AIエージェントでテンプレートを作る

Shojikuのテンプレートは、AIエージェントが読み書きできるように設計されています。帳票PDFの入力になるのは、YAMLとJSONのテキストです。エラーには安定した診断コードが付き、レイアウトの結果はツリーとして読み返せます。だからエージェントは、書いて確かめるサイクルを自分で回せます。
...
このサイトの各ページには`.md`のプレーンテキスト版があります。[/llms.txt](/llms.txt)が目次で、[/llms-full.txt](/llms-full.txt)には、テンプレートリファレンスが丸ごと入っています。索引と診断コードだけでなく機能ページも全部あるので、`flex`について知りたいエージェントは追加で取りに行かずに済みます。エージェントにShojikuを教えるときは、このURLを渡すだけで済みます。
