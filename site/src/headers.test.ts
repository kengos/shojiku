...
describe("_headers", () => {
  ...
  it("site scope carries exactly the site's additions", () => {
    ...
    // The token becomes per-build sha256 hashes (inject-csp-hashes.ts) — the
    // hash-allowlist form of "no 'unsafe-inline' for scripts".
    expect(siteCsp).toContain(
      "script-src 'self' 'wasm-unsafe-eval' https://static.cloudflareinsights.com __INLINE_SCRIPT_HASHES__",
    );
