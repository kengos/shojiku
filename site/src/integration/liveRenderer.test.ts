...
import { GRID_KNOB_DEFAULTS, gridDemoTemplate, TEXT_KNOB_DEFAULTS, textDemoTemplate } from "../lib/playground.ts";
...
function tierSource(tierName: (typeof TIERS)[number]["tier"]): TierSource {
  const t = TIERS.find((x) => x.tier === tierName)!;
...
describe("immediate tier (en-US)", () => {
  it("renders receipt-us warning-free on the noto-sans subset alone", async () => {
    ...
    await injectTier(e, tierSource("immediate"));
    loadLocale(e, "en-US");
...
describe("ja tier gate", () => {
  it("receipt-ja renders clean only after the biz-ud tier is injected", async () => {
    ...
    await injectTier(e, tierSource("lazy-ja"));
