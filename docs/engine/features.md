...
- **Shipped locale packs**: zh-TW, zh-CN, hi-IN, fil-PH and th-TH ship as
...
- License: triple **MIT OR Apache-2.0 OR BSD-3-Clause**. Bundled
  ...
  showcase), the seven locale receipts
  (`receipt-ja`/`-us`/`-zh-tw`/`-zh-cn`/`-hi-in`/`-fil-ph`/`-th-th`),
...
- **Thai segmentation ships unconditionally, not behind a Cargo
  feature.** Gating it per deployment was the obvious way to keep the
  segmenter's data out of the WASM bundle, and it is the wrong shape: a
  feature that changes LINE BREAKING would make the same template and
  ...
  call to a pure-Rust `libm` with no platform branch. The constructor is
  named explicitly rather than taking ICU4X's "best available data", so
  a future release of it cannot quietly spend the rest of the budget.
- **A new locale is a pack, not a builtin.** Enabling zh-TW/zh-CN/hi-IN/
  ...
  th-TH is the first pack that did not come entirely free, and the
  exception proves the shape rather than breaking it: its Buddhist era
  begins before year 1, and the era-start parser split on `-`, so a
  signed year could not be authored. One parser function, no new wire.
