...
- **Writing or fixing templates?** Start at [engine/](engine/README.md) —
  ...
  and the render commands. It is the "what can I write?" surface, and
...
- **Evaluating or embedding the engine?** Start at the
  [quickstart](quickstart.md) — clone → build (or Docker) → render →
  MCP registration. Today's integration surfaces
...
Last full doc audit (`shojiku-document-curator`): commit `389b6f69`.
