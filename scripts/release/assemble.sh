...
#
#   scripts/release/assemble.sh [python|ruby|npm|dotnet|java|checksums ...]
#
...
# Packagist) and go publishes as a git tag, so neither appears here.
#
...
assemble_checksums() {
  ...
  (cd "$OUT" && find . -type f ! -name SHA256SUMS -exec $SHA {} + | sort -k2 > SHA256SUMS)
  wc -l < "$OUT/SHA256SUMS" | xargs echo "  files:"
}

require_bin
[ $# -gt 0 ] || set -- python ruby npm dotnet java checksums
for component in "$@"; do
  "assemble_$component"
done
