...
| Rust crates | not yet published | going up as `shojiku-*` on crates.io — `shojiku-cli` (the tool) and `shojiku-authoring` (the embedding surface) |

Until those four land, the Docker image is the way to run Shojiku without
a language SDK.
