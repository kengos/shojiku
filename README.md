...
[docs/quickstart.md](docs/quickstart.md) walks the whole path: get the
CLI (build from source, or the Docker image), render a bundled
example, and register the stdio MCP server (`shojiku-mcp` — validate /
render_preview / inspect_layout / capabilities) so an AI coding agent
can drive the engine. The agent-first loop in one line — register the
server and ask:
...
| `make docker-build` / `make docker-render` | Build the runtime image (`shojiku-ci:local`) / render the bundled example through it — the no-Rust path to a PDF ([quickstart](docs/quickstart.md)) |
