...
- **GitHub CI is currently disabled (private repo, Actions billing-blocked) — do NOT wait on or gate anything on `gh pr checks`**; red checks are runner-never-started noise. The LOCAL `make` gates above are the merge bar. Details + when to lift this note: shojiku-release-engineer skill → Ship mechanics.
