...
- No local cargo: run gates via `make` (Docker wrapper). **The merge bar is CI, not a local full run** (user decision): `make verify` still exists and is still the whole mirror (line budget, fmt, clippy -D warnings, test, coverage **100% lines blocking**, cargo-deny, the key-catalog drift gate (`make reference-check`; regenerate via `make reference-data`), examples output-hash check (`make examples-check`; refresh via `make examples`), docker build/render/trivy), but it is NOT the routine pre-PR step — CI runs a strict superset of it (see below) in parallel, in minutes rather than the ~20 a local serial run costs. Reach for it only when you cannot use CI: working offline, or changing a gate's own recipe and wanting the answer before pushing.
