...
CARGO_IN_DOCKER = docker run --rm \
	...
	-v shojiku-cargo:/usr/local/cargo \
	-v shojiku-rustup:/usr/local/rustup \
	$(RUST_IMAGE) sh -euc
...
PHP_IMAGE := shojiku-sdk-php:local
...
sdk-php: cli-bin ## sdk/php gates: php-cs-fixer + phpstan + phpunit at 100% coverage + composer install
	@echo "== sdk php image =="
	@DOCKER_BUILDKIT=1 docker build -q -f sdk/php/Dockerfile -t $(PHP_IMAGE) . >/dev/null
...
sdk-go: cli-bin ## sdk/go gates: gofmt + vet + golangci-lint + go test -race at 100% coverage + module build
	@echo "== sdk go image =="
	@DOCKER_BUILDKIT=1 docker build -q -f sdk/go/Dockerfile -t $(GO_IMAGE) . >/dev/null
...
RUBY_IMAGE := shojiku-sdk-ruby:local
...
napi: ## Build the Node addon (engine/napi with the shim feature) into dist/napi/local
	...
	@docker run --rm -v "$(CURDIR)/$(NAPI_LOCAL):/addon:ro" $(NODE_FLOOR_IMAGE) \
		node -e 'const a=require("/addon/shojiku.node"); \
		if (a.abiVersion() !== 1) { throw new Error("unexpected ABI revision"); } \
		console.log("addon loads; abi", a.abiVersion());'
...
sdk-ruby: capi-lib ## sdk/ruby gates: rubocop + rspec at 100% coverage + gem build/install
	@echo "== sdk ruby image =="
	@DOCKER_BUILDKIT=1 docker build -q -f sdk/ruby/Dockerfile -t $(RUBY_IMAGE) . >/dev/null
...
PYTHON_IMAGE := shojiku-sdk-python:local
...
sdk-python: capi-lib ## sdk/python gates: ruff + mypy + pytest at 100% coverage + wheel build/install
	...
	@DOCKER_BUILDKIT=1 docker build -q -f sdk/python/Dockerfile -t $(PYTHON_IMAGE) . >/dev/null
...
DOTNET_IMAGE := shojiku-sdk-dotnet:local
...
sdk-dotnet: capi-lib ## sdk/dotnet gates: dotnet format + xunit at 100% line coverage + pack/restore
	...
	@DOCKER_BUILDKIT=1 docker build -q -f sdk/dotnet/Dockerfile -t $(DOTNET_IMAGE) . >/dev/null
...
JAVA_IMAGE := shojiku-sdk-java:local
...
sdk-java: capi-lib ## sdk/java gates: spotless + junit at 100% line coverage + jar/sources/javadoc
	@echo "== sdk java image =="
	@DOCKER_BUILDKIT=1 docker build -q -f sdk/java/Dockerfile -t $(JAVA_IMAGE) . >/dev/null
...
sdk-js: napi ## sdk/js gates: biome + tsc + vitest at 100% coverage + pack/install
	@echo "== sdk js image =="
	@DOCKER_BUILDKIT=1 docker build -q -f sdk/js/Dockerfile -t $(JS_IMAGE) . >/dev/null
...
# corepack activates the pnpm version pinned in gui/package.json.
...
gui: gui-budget ## gui/ workspace gates: line budget + typecheck + lint (0 warnings) + test/coverage
	...
	@$(PNPM_IN_DOCKER) 'corepack enable; \
...
gui-serve: ## Build the Designer-app image and serve it (http://localhost:8788, Ctrl-C stops)
	...
	docker build -f gui/designer-app/Dockerfile -t $(GUI_APP_IMAGE) .
...
gui-dev: ## Vite dev server (HMR) in Docker for gui/ work (http://localhost:5173, Ctrl-C stops)
	...
	docker run --rm -it -p $(GUI_DEV_PORT):5173 \
		...
		-v shojiku-pnpm:/pnpm-store \
		-e COREPACK_ENABLE_DOWNLOAD_PROMPT=0 \
		$(NODE_IMAGE) sh -euc 'corepack enable; \
		pnpm config set store-dir /pnpm-store; \
		pnpm install --frozen-lockfile; \
