...
thresholds. All gates run in Docker via `make gui:verify` (no host toolchain), and
`make gui:verify` is part of `make verify`.

```sh
make gui:verify        # typecheck + lint (0 warnings) + test/coverage, in Docker
...
work (same pnpm store volume as the `make gui:verify*` targets).
