...
export const COPY_JA: Record<string, string> = {
  ...
  'ch0.margin':
    '同じ画面の「余白」を 24 にします。長さの単位は pt で、1pt はおよそ 0.35mm、A4 の幅が 595pt です。',
  ...
  'ch7.pin':
    '配置タブの「自動/固定」で「固定」を押します。見た目はそのままで、以後この画像は他の要素が増減しても動かなくなります（「自動」を押せばいつでも元の並びに戻せます）。',
  'ch7.move':
  ...
  'ch8.export': '「ファイル」→「書き出す」で、完成した文書を出力します。',
  'ch8.done':
  ...
  'topic-placement.pin':
    '左のテキストを選び、配置タブの「自動/固定」で「固定」を押します。見た目はそのままで、以後この要素は他が増減しても動かなくなります。',
  ...
  'topic-style.origin':
    '「御請求書」をクリックすると、右パネルの各項目に由来バッジが付きます。サイズは「スタイル『タイトル』」、色は「既定値」です。スタイル、継承、既定の3種で値の出どころを見分けられます。',
  'topic-style.update':
