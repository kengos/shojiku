...
export const COPY_JA: Record<string, string> = {
  'ch0.blank':
    '練習用の白紙を開きました。ここで何をしても、あなたが開いていた文書には影響しません。',
  'ch0.pageSize':
    '右の「文書設定を開く」を押すと、紙面ぜんたいの設定が開きます。「ページ設定」で用紙が A4・縦になっていることを確認してください。',
  'ch0.margin':
    '同じ画面の「余白」を 24 にします。長さの単位は pt です — 1pt はおよそ 0.35mm、A4 の幅が 595pt です。',
  ...
  'ch1.style':
    'この見た目に名前を付けて使い回せます — 「スタイル」→「選択からスタイル登録」で「タイトル」として保存します。',
  'ch2.openPicker':
  ...
  'ch2.gap':
    'スロットを選ぶと右パネルの上に「親コンテナ」カードが現れます — その「間隔」を 8 にします(スロットのあいだの空きです)。',
  'ch2.ratio':
    '「比率」を左:中:右の順で 1:2:1 にします。数字は各スロットが取る横幅の割合で、真ん中が2倍の幅になります。',
  'ch2.auto':
    '配置タブの X/Y が薄い字で「自動」と出ています。位置は道具が計算するので、あなたが数字を決める必要はありません — 数字で決めるのは、第6章の「毎ページ同じ帯」と第7章の「判子」だけです。',
  'ch3.openField':
    '発行のたびに変わる値(宛名・日付・金額)は「データ項目」にします — 「挿入」→「データ項目を作成…」を開きます。',
  'ch3.create':
    '「customer」という名前で、テキストのデータ項目をひとつ作ります。項目名は半角の英字にします — 文章の途中に差し込むとき、日本語の名前は差し込めないためです。',
  'ch3.dataTab': '左の「データ項目」タブを開きます。いま作った項目が並んでいます。',
  'ch3.bind': '「customer」を紙面へドラッグします。文字がサンプルの値に変わります。',
  'ch3.chip':
    '「請求日: 」の文中に「date」をドラッグします。文章の途中に、値の断片(チップ)として入ります。',
  'ch3.sample':
    '「データ項目」タブの歯車ボタンから編集画面を開き、宛名の値を書き換えてみます。紙面がすぐ追従します。',
  'ch3.format': '金額の「表示形式」で symbol を選びます。¥300,000 のような通貨表示になります。',
  'ch4.openIterable':
    '明細のように行数が毎回変わるくり返しのデータは「一覧データ」で置きます — 「挿入」→「一覧データを置く…」を選びます。',
  'ch4.create':
    '「items」という名前で、品名(name)・数量(qty)・単価(price)・金額(amount)を持つ一覧を作り、「表」の形で置きます。',
  'ch4.drawn': 'サンプルの行数ぶん表が描かれ、いちばん上に見出しの行(ヘッダー行)が付きました。',
  'ch4.width': '右の「列」で、金額列の「列幅」を 90 にします。',
  'ch4.alignRight':
    '数量・単価・金額の列を、1列ずつ選んで右揃えにします。数字は右揃えが読みやすい定石です。',
  'ch4.paginate':
    'データ項目の編集画面で明細（items）の行をどんどん増やしてみます。1ページに収まらなくなると、紙面の下に2ページ目が現れて続き、見出し行も繰り返されます。',
  ...
  'ch5.bold':
    '合計の文字を太字にします。スタイル登録するほどではない一回きりの指定は、直接でかまいません。',
  'ch6.selectFooter':
    'どのページにも同じ位置に印刷される帯を「フッター」(下端)・「ヘッダー」(上端)と呼びます — 左の「構成」タブ(要素の重なりの一覧)でフッターを選択します。',
  ...
  'ch6.everyPage': 'どのページにも同じフッターが出ていることを確かめてください。',
  'ch7.image': '「挿入」→「画像を配置…」で社判の画像を選びます。',
  'ch7.pin':
    '配置タブの「自動/固定」で「固定」を押します。見た目はそのまま、以後この画像は他の要素が増減しても動かなくなります(「自動」を押せばいつでも元の並びに戻れます)。',
  'ch7.move':
    'X 480 / Y 40 にします。紙面をドラッグしても動かせます — ずれても ⌘Z でいつでも戻せます。',
  'ch8.diagnostics':
    '画面下の警告一覧(診断)が空になっているか確認します。問題があればここに日本語で並び、クリックで該当箇所に飛べます。',
  'ch8.sample':
    'データ項目の編集画面でサンプルデータを一度差し替えて、全体が追従することを確認します。',
  'ch8.export': '「ファイル」→「書き出す」で成果物を書き出します。',
  'ch8.done':
  ...
  'topic-containers.nest':
    'スロットを選んだまま「挿入」→「コンテナ…」を入れると、その区画の中に入れ子で入ります(挿入先が予告されます)。',
  ...
  'topic-placement.explain':
    'この文書のヘッダーは「コンテナ」の中に3つのテキストが並んでいます。中の要素の位置は道具が自動で計算します(配置タブの X/Y が薄い字で「自動」)。',
  'topic-placement.pin':
    '左のテキストを選び、配置タブの「自動/固定」で「固定」を押します。見た目はそのまま、以後この要素は他が増減しても動かなくなります。',
  'topic-placement.move':
    'X/Y の数字を変えると、固定した要素だけが動きます。紙面をドラッグしても動かせます — ずれても ⌘Z で戻せます。',
  'topic-placement.unpin':
    'もう一度「自動」を押すと固定が解除され、元の並びに戻ります(再フロー)。位置は道具が計算し直します。',
  'topic-style.origin':
    '「御請求書」をクリックすると、右パネルの各項目に由来バッジが付きます — サイズは「スタイル『タイトル』」、色は「既定値」。3種(スタイル/継承/既定)で値の出どころを見分けられます。',
  'topic-style.update':
    'まず「タイトル」スタイル自体を更新します。スタイルピッカーの「更新」からサイズを変えると、それを使っている箇所がまとめて追従します。',
  'topic-style.override':
    '次にタイトルを選び、書式バーでサイズを直接変えます。由来が「スタイル」から「この要素」に変わり、以後この要素はスタイルに追従しません — 上書きはこの要素だけに効きます。',
};
...
export const TOPIC_SUBTITLES_JA: Record<string, string> = {
  'topic-containers': '縦積み・表組み・ネスト・スロット追加',
  'topic-binding': 'バインドの付け替え・チップ挿入',
  'topic-table': 'Excel から表を貼り付け',
  'topic-footer': 'ヘッダー帯・ページ番号',
  'topic-placement': '固定⇔自動を切り替えて再フロー',
