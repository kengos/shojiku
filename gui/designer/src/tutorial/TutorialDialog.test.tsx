...
// utility classes.
describe('TutorialDialog — emphasis (Material 3: one primary per screen)', () => {
  it('paints its confirming action as the primary, and its dismissal as a peer', () => {
    draw({ completed: [], dismissed: false });
    expect(screen.getByTestId('tutorial-resume').dataset.variant).toBe('primary');
    expect(screen.getByRole('button', { name: 'Clear progress' }).dataset.variant).toBe('ghost');
  });
});
