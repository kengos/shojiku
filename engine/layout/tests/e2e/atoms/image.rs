//! Image atoms end to end: `object-fit` placement, the clip that crops
//! an overflowing fit, and the clip an SVG gets under every fit (its
//! paths may leave the `viewBox`, and the viewport clips).
...
"#,
    ...
    assert!(diags.is_empty(), "diagnostics: {diags:?}");
    // `stretch` fills the box exactly, so nothing overflows — but the
    // asset is an SVG, whose paths may sit outside the viewBox, so it is
    // clipped to the content box anyway.
    let (clip, img) = clipped_image(&doc);
    assert_eq!(img.asset_id, "dyn:qr");
    assert_eq!((img.x, img.y, img.w, img.h), (0.0, 0.0, 30.0, 60.0));
    assert_eq!((clip.x, clip.y, clip.w, clip.h), (0.0, 0.0, 30.0, 60.0));
}

#[test]
fn svg_is_clipped_to_the_content_box_even_when_the_fit_cannot_overflow() {
    let assets = test_assets();
    let (doc, diags) = run_with_assets(
        r#"
page: { margin: 0 }
...
"#,
    ...
    assert!(diags.is_empty(), "diagnostics: {diags:?}");
    // A 20x10 viewBox contained in a 100x40 box: scale 4 -> 80x40,
    // centered in x. The drawn rect sits inside the box, so the overflow
    // test alone said "no clip" — and a path outside the viewBox then
    // painted over the rest of the page. The clip is the viewport.
    let (clip, img) = clipped_image(&doc);
    assert_eq!((img.x, img.y, img.w, img.h), (10.0, 0.0, 80.0, 40.0));
    assert_eq!((clip.x, clip.y, clip.w, clip.h), (0.0, 0.0, 100.0, 40.0));
